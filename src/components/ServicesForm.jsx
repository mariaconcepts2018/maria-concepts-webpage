...
const sevices = ['modular kitchen', 'wardrobe', 'TV unit', 'False cielings', 'Painting', 'Doors and Furnitures'];

export default function ServicesForm({setStep, setError, formData, setFormData, setMessage}){
  ...
  const handleSubmit = async () => {
          ...
          try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/api/update-user`, { // Replace with your API route
              method: 'POST',
              headers: {
            ...
            setStep(8);
            setFormData({projectType: "", floors:"", houseType: "", services: [], layout: ""})
...
{sevices.map((item, index) => (
  <button
    ...
    value={item}
    onClick={handleClick}
    className={`${selected.includes(item) ? 'border-primary-500 ' : 'border-gray-300 '} border text-neutral-600 capitalize  px-2 py-1 rounded-lg transition cursor-pointer
        "
        }`}
    >
        {item}
